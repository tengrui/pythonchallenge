"""
http://www.pythonchallenge.com/pc/def/map.html
http://www.pythonchallenge.com/pcc/def/ocr.html
"""
import string

fromTab = "abcdefghijklmnopqrstuvwxyz"
toTab = "cdefghijklmnopqrstuvwxyzab"

input = "g fmnc wms bgblr rpylqjyrc gr zw fylb. rfyrq ufyr amknsrcpq ypc dmp. bmgle gr gl zw fylb gq glcddgagclr ylb rfyr'q ufw rfgq rcvr gq qm jmle. sqgle qrpgle.kyicrpylq() gq pcamkkclbcb. lmu ynnjw ml rfc spj. "
url = "map"

transMap = str.maketrans(fromTab, toTab)

print(input.translate(transMap))
print("http://www.pythonchallenge.com/pc/def/" + url.translate(transMap) + ".html")